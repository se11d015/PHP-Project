...
$valueQuery = "tgaof.*,  st_x(tgaof.geom) as geomx, st_y(tgaof.geom) as geomy,  takn.kingdom_code, takn.kingdom_name, takn.kingdom_name_mn, takn.kingdom_name_en, takn.kingdom_name_ru, 
tapn.phylum_code, tapn.phylum_name, tapn.phylum_name_mn, tapn.phylum_name_en, tapn.phylum_name_ru, tacn.class_code, tacn.class_name, tacn.class_name_mn, tacn.class_name_en, tacn.class_name_ru, 
taon.order_code, taon.order_name, taon.order_name_mn, taon.order_name_en, taon.order_name_ru, tafn.family_code, 
tafn.family_name, tafn.family_name_mn, tafn.family_name_en, tafn.family_name_ru, tagn.genus_code, tagn.genus_name, 
tagn.genus_name_mn, tagn.genus_name_en, tagn.genus_name_ru, tapl.species_name, tapl.species_name_mn,  
tapl.species_name_en, tapl.species_name_ru FROM " . $schemas . ".takingdomname takn," . $schemas . ".taphylumname tapn," . $schemas . ".taclassname tacn," . $schemas . ".tafamilyname tafn," . $schemas . ".taordername taon," . $schemas . ".tagenusname tagn," . $schemas . ".taanimalname tapl," . $schemas . ".tganimaloffence tgaof ";

	if ($sess_profile == 1) {
		$whereQuery = "WHERE tapl.species_code = tgaof.species_code AND tapl.genus_code = tagn.genus_code AND tagn.family_code = tafn.family_code AND tafn.order_code = taon.order_code AND taon.class_code=tacn.class_code AND tacn.phylum_code=tapn.phylum_code  AND tapn.kingdom_code=takn.kingdom_code  AND tgaof.offence_id = " . $offence_id;
	} 
	else {
		$whereQuery = "WHERE  tapl.species_code = tgaof.species_code AND tapl.genus_code = tagn.genus_code AND tagn.family_code = tafn.family_code AND tafn.order_code = taon.order_code AND taon.class_code=tacn.class_code AND tacn.phylum_code=tapn.phylum_code  AND tapn.kingdom_code=takn.kingdom_code  AND tgaof.offence_id = " . $offence_id . " AND tgaof.user_id = " . $sess_user_id;
...
function updatesubmitform()
{
    if (document.getElementById("place_name").value == "") {
        ...
        document.getElementById("updateoffencebttn").value = "1";
...
<table class="table table-bordered table-condensed">
  <thead>
    <tr>
      <th><?php echo getdata($ITEM_TYPE, 100); ?> засварлах хэсэг</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><form class="form-horizontal" action="<?php echo $my_url . $search_url . $sort_url; ?>" enctype="multipart/form-data" method="post" name="mainform" id="mainform">
          <input type="hidden" name="offence_id" id="offence_id" value="<?php echo $row[$i]["offence_id"]; ?>">
         ...
         <div class="control-group">
          ...
          <div class="control-group ">
            <label class="control-label" id="lbl">Зөрчил гарсан огноо:</label>
            <div class="controls">
              <?php
                            $myCalendar1 = new tc_calendar("offence_date", true);
                            $offence_date = $row[$i]["offence_date"];
                            $myCalendar1->setIcon("calendar/images/iconCalendar.gif");
                            $myCalendar1->setDate(date('d', strtotime($offence_date)), date('m', strtotime($offence_date)), date('Y', strtotime($offence_date)));
          ...
          <div class="control-group ">
            <label class="control-label" id="lbl">Зөрчил гарсан газрын нэр:</label>
            <div class="controls">
          ...
          <div class="control-group ">
            <label class="control-label" id="lbl">Хууль бусаар агнасан, барьсан амьтны тоо:</label>
            <div class="controls">
              <input type="text" name="offense_amount" id="offense_amount" class="span4"  value="<?php echo $row[$i]["offense_amount"]; ?>"/>
		  ...
		  <div class="control-group">
              ...
              <div class="controls">
                <?php
					echo seldata("amount_unit", "span2", $OFFENCE_TYPE, $row[$i]["amount_unit"]);
					?>
                <span class="help-inline">Жагсаалтаас аль нэгийг нь сонгоно.</span></div>
            </div>
          <div class="control-group ">
            <label class="control-label" id="lbl">Ногдуулсан торгуулийн хэмжээ, мян төг:</label>
            <div class="controls">
              <input type="text" name="penalty_payment" id="penalty_payment" class="span4"  value="<?php echo $row[$i]["penalty_payment"]; ?>"/>
               <span class="help-inline">15 орноос хэтрэхгүй бутархай тоо байна. </span> </div>
          </div>
          <div class="control-group ">
            <label class="control-label" id="lbl">Нөхөн төлбөрийн хэмжээ, мян төг:</label>
            <div class="controls">
              <input type="text" name="recovery_payment" id="recovery_payment" class="span4"  value="<?php echo $row[$i]["recovery_payment"]; ?>"/>
              <span class="help-inline">15 орноос хэтрэхгүй бутархай тоо байна. </span> </div>
          </div>
          <div class="control-group ">
          ...
          <input type="hidden" id="updateoffencebttn" name="updateoffencebttn" value="0"/>
