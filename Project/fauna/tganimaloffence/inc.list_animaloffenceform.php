<?php
if (isset($_POST["searchoffencebttn"])) {

$searchQuery = "";
$search_url = "";

$offence_date = (isset($_POST["offence_date"])) ? (int) $_POST["offence_date"] : 0;
...
if (isset($_GET["sort"])) {
 if ($_GET["sort"]==3)
    ...
    $sortQuery .= "tasou.soum_name_mn";
	  ...
	  elseif ($_GET["sort"] == 8) {
    $sortQuery .= "tgaof.offence_date";
	} 
	else {
    $sortQuery .= " tgaof.offence_id";
}
} else {
$sortQuery .= " tgaof.offence_id";
}
...
$startQuery = "SELECT";
$valueQuery = "COUNT(*) AS num_count FROM scadministrative.taaimagname taaim, scadministrative.tasoumname tasou, ".$schemas . ".tganimaloffence tgaof,".$schemas.".takingdomname takn,".$schemas.".taphylumname tapn,".$schemas.".taclassname tacn,".$schemas.".tafamilyname tafn,".$schemas.".taordername taon,".$schemas.".tagenusname tagn,".$schemas.".taanimalname tapl";
		$whereQuery = "WHERE taaim.aimag_code=tgaof.aimag_name AND tasou.soum_code=tgaof.soum_name AND tgaof.offence_id=tgaof.offence_id AND tapl.species_code = tgaof.species_code AND tapl.genus_code = tagn.genus_code AND tagn.family_code = tafn.family_code AND tafn.order_code = taon.order_code AND taon.class_code=tacn.class_code AND tacn.phylum_code=tapn.phylum_code  AND tapn.kingdom_code=takn.kingdom_code";
...
require("tganimaloffence/inc.search_animaloffenceform.php");
...
<div class="list-table">
  <table class="table table-bordered table-striped table-hover">
    <thead>
      <tr>
        <th colspan="9"><span class="title"><?php echo getdata($ITEM_TYPE, 100); ?></span></th>
      </tr>
    </thead>
    <thead>
      <tr>
	   ...
	   <th class="span4"><a href="<?php echo $my_url."&sort=5&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="  icon-tag icon-white"></i>Амьтны монгол нэр</a></th>
        ...
        <th class="span4"><a href="<?php echo $my_url . "&sort=8&sorttype=" . $sorttype . $search_url; ?>" style="color:#FFFFFF"><i class="  icon-tag icon-white"></i> Зөрчил гарсан огноо</a></th>
        <th class="span4"><a href="<?php echo $my_url . "&sort=8&sorttype=" . $sorttype . $search_url; ?>" style="color:#FFFFFF"><i class="  icon-tag icon-white"></i> Зөрчил гарсан газрын нэр</a></th>		
		<th class="span3"></th>
      </tr>
    </thead>
    <tbody>
...
?>
      <tr>
       ...
       <td><?php echo $rows[$i]["offence_date"]; ?></td>
	   <td><?php echo $rows[$i]["place_name"]; ?></td>
        <td align="center"><a href="<?php echo $my_url . $my_page . $search_url . $sort_url . "&action=more&offence_id=" . $rows[$i]["offence_id"]; ?>" title="Дэлгэрэнгүй харах"><i class="icon-list"></i></a>
          <?php
if ($sess_profile == 1) {
    ?>
          <a href="<?php echo $my_url . $my_page . $search_url . $sort_url . "&action=edit&offence_id=" . $rows[$i]["offence_id"]; ?>" title="Засварлах"><i class="icon-edit"></i></a> <a href="<?php echo $my_url . $search_url . $sort_url . "&action=delete&offence_id=" . $rows[$i]["offence_id"]; ?>" title="Устгах" onClick="return confirm('Та устгахдаа итгэлтэй байна уу?');"><i class="icon-trash"></i></a>
          <?php
            } else if ($rows[$i]["user_id"] == $sess_user_id) {
                ?>
          <a href="<?php echo $my_url . $my_page . $search_url . $sort_url . "&action=edit&offence_id=" . $rows[$i]["offence_id"]; ?>" title="Засварлах"><i class="icon-edit"></i></a> <a href="<?php echo $my_url . $search_url . $sort_url . "&action=delete&offence_id=" . $rows[$i]["offence_id"]; ?>" title="Устгах" onClick="return confirm('Та устгахдаа итгэлтэй байна уу?');"><i class="icon-trash"></i></a></td>
        <?php
            }
            ?>
          </td>
