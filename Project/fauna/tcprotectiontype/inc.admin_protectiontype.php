...
if($sess_profile==1)
{
	if(isset($_POST["inserttypebttn"]))
	{
		if(isset($_POST["type_name"]) && strlen($_POST["type_name"])>0)
		{
			...
			$result = $db->insert("".$schemas.".tcprotectiontype", $fields, $values);
	...
	{
		if(isset($_POST["type_id"]) && isset($_POST["type_name"]) && strlen($_POST["type_name"])>0)
		{	
			...
			$result = $db->update("".$schemas.".tcprotectiontype", $fields, $values, $wherevalues);
			if(! $result)
	...
	if(($action=="delete") && isset($_GET["typeid"]))
	{
		...
		$result = $db->delete("".$schemas.".tcprotectiontype", $wherevalues);
		if(! $result)
...
$selQuery = "SELECT tct.* FROM ".$schemas.".tcprotectiontype tct";
...
<form action="<?php echo $my_url; ?>" method="post" name="mainform" id="mainform">
  <div class="list-table">
    <table class="table table-bordered table-hover">
      <thead>
        <tr>
          <th colspan="3"><span class="title"><?php echo getdata($ITEM_TYPE, 53); ?></span></th>
        </tr>
      </thead>
      <thead>
        <tr>
          <th class="span1">№</th>
          <th class="span4">Арга хэмжээний нэр</th>
          <th class="span6"></th>
        </tr>
      </thead>
      <tbody>
        <?php
				...
				$selQuery = "SELECT tct.* FROM ".$schemas.".tcprotectiontype tct ORDER BY tct.type_name ASC LIMIT ".$limit;
				$rows = $db->query($selQuery);
        ...
        <tr>
          ...
          <td><div class="controls">
              <input type="text" class="input-xlarge" id="type_name" name="type_name" value="<?php echo $rows[$i]["type_name"]; ?>" >
            </div></td>
        ...
        <tr>
          <td></td>
          <td><div class="controls">
              <input type="text" class="input-xlarge" id="type_name" name="type_name" >
            </div></td>
