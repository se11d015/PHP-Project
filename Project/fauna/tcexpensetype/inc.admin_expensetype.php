...
if($sess_profile==1)
{
	if(isset($_POST["insertexpensetypebttn"]))
	{
		if(strlen($_POST["type_name"])>0)
		{
			...
			$result = $db->insert("".$schemas.".tcexpensetype", $fields, $values);
	...
	if(isset($_POST["updateexpensetypebttn"]))
	{
		if(strlen($_POST["type_name"])>0)
		{	
			...
			$result = $db->update("".$schemas.".tcexpensetype", $fields, $values, $wherevalues);
			if(! $result)
	...
	if(($action=="delete") && isset($_GET["type_id"]))
	{
		...
		$result = $db->delete("".$schemas.".tcexpensetype", $wherevalues);
		if(! $result)
...
$selQuery = "SELECT tcrt.* FROM ".$schemas.".tcexpensetype tcrt";
...
<form action="<?php echo $my_url; ?>" method="post" name="mainform" id="mainform">
  <div class="list-table">
    <table class="table table-bordered table-hover">
      <thead>
        <tr>
          <th colspan="7"><span class="title"><?php echo getdata($ITEM_TYPE, 52); ?></span></th>
        </tr>
      </thead>
      <thead>
        <tr>
          <th class="span1">№</th>
          <th class="span2">Зардлын эх үүсвэрийн нэр</th>
          <th class="span2"></th>
        </tr>
      </thead>
      <tbody>
        <?php
				...
				$selQuery = "SELECT tcrt.* FROM ".$schemas.".tcexpensetype tcrt ORDER BY tcrt.type_id ASC LIMIT ".$limit;
				////echo $selQuery;
				$rows = $db->query($selQuery);
        ...
        <tr>
          ...
          <td><div class="controls">
              ...
              <input type="text" id="type_name" name="type_name" value="<?php echo $rows[$i]["type_name"]; ?>" >
            </div></td>
          </td>
          <td align="center"><button type="submit" class="btn btn-danger" name="updateexpensetypebttn">Хадгалах</button>
        ...
        <tr>
          ...
          <td align="center"><button type="submit" class="btn btn-danger" name="insertexpensetypebttn">Хадгалах</button>
            <a class="btn btn-danger" href="<?php echo $my_url.$my_page; ?>">Болих</a></td>
        </tr>
        <?php
				}
