...
if($sess_profile==1)
{
	if(isset($_POST["insertresearchtypebttn"]))
	{
		if(isset($_POST["type_name"]))
		{
			...
			$result = $db->insert("".$schemas.".tcresearchtype", $fields, $values);
	...
	if(isset($_POST["updateresearchtypebttn"]))
	{
		...
		{	
			...
			$result = $db->update("".$schemas.".tcresearchtype", $fields, $values, $wherevalues);
			if(! $result)
	...
	if(($action=="delete") && isset($_GET["type_id"]))
	{
		...
		$result = $db->delete("".$schemas.".tcresearchtype", $wherevalues);
		if(! $result)
...
$selQuery = "SELECT tcrt.* FROM ".$schemas.".tcresearchtype tcrt";
...
<form action="<?php echo $my_url; ?>" method="post" name="mainform" id="mainform">
  <div class="list-table">
    <table class="table table-bordered table-hover">
      <thead>
        <tr>
          <th colspan="3"><span class="title"><?php echo getdata($ITEM_TYPE, 32); ?></span></th>
        </tr>
      </thead>
      <thead>
        <tr>
          <th class="span1">№</th>
          <th class="span4">Судалгааны төрлийн нэр</th>
          <th class="span3"></th>
        </tr>
      </thead>
      <tbody>
        <?php
			...
			$selQuery = "SELECT tcrt.* FROM ".$schemas.".tcresearchtype tcrt ORDER BY tcrt.type_id ASC LIMIT ".$limit;
			////echo $selQuery;
			$rows = $db->query($selQuery);
        ...
        <tr>
          ...
          <td align="center"><button type="submit" class="btn btn-danger" name="updateresearchtypebttn">Хадгалах</button>
        ...
        <tr>
          ...
          <td align="center"><button type="submit" class="btn btn-danger" name="insertresearchtypebttn">Хадгалах</button>
            <a class="btn btn-danger" href="<?php echo $my_url.$my_page; ?>">Болих</a></td>
        </tr>
        <?php
			}
