...
if($sess_profile==1)
{
	if(isset($_POST["inserttypebttn"]))
	{
		if(isset($_POST["type_name"]))
		{
			...
			$result = $db->insert("".$schemas.".tcreporttype", $fields, $values);
	...
	if(isset($_POST["updatetypebttn"]))
	{
		...
		{	
			...
			$result = $db->update("".$schemas.".tcreporttype", $fields, $values, $wherevalues);
			if(! $result)
	...
	if(($action=="delete") && isset($_GET["typeid"]))
	{
		...
		$result = $db->delete("".$schemas.".tcreporttype", $wherevalues);
		if(! $result)
...
$selQuery = "SELECT tct.* FROM ".$schemas.".tcreporttype tct";
...
<form action="<?php echo $my_url; ?>" method="post" name="mainform" id="mainform">
  <div class="list-table">
    <table class="table table-bordered table-hover">
      <thead>
        <tr>
          <th colspan="3"><span class="title"><?php echo getdata($ITEM_TYPE, 43); ?></span></th>
        </tr>
      </thead>
      <thead>
        <tr>
          <th class="span1">№</th>
          <th class="span4">Ажлын тайлангийн төрөл</th>
          <th class="span6"></th>
        </tr>
      </thead>
      <tbody>
        <?php
			...
			$selQuery = "SELECT tct.* FROM ".$schemas.".tcreporttype tct ORDER BY tct.type_name ASC LIMIT ".$limit;
			$rows = $db->query($selQuery);
