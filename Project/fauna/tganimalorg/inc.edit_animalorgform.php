...
if($db->isGroupRole($schemas, $sess_profile, $sess_user_id, 7, 2))
{
	...
	$startQuery = "SELECT";
	$valueQuery = "tgpo.*,  st_x(tgpo.geom) as geomx, st_y(tgpo.geom) as geomy FROM ".$schemas.".tganimalorg tgpo";
	
	if($sess_profile==1)
		$whereQuery = "WHERE tgpo.org_id = ".$org_id;
	else
		$whereQuery = "WHERE tgpo.org_id = ".$org_id." AND tgpo.user_id = ".$sess_user_id;
...
function updatesubmitform()
{
	if (document.getElementById("org_name").value==""){
		...
		document.getElementById("updateanimalorgbttn").value = "1";
...
<div class="add-table">
  <table class="table table-bordered table-condensed">
    <thead>
      <tr>
        <th><?php echo getdata($ITEM_TYPE, 40); ?> засах хэсэг</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><form class="form-horizontal" action="<?php echo $my_url.$search_url.$sort_url; ?>" enctype="multipart/form-data" method="post" name="mainform" id="mainform">
            <input type="hidden" name="org_id" id="org_id" value="<?php echo $row[$i]["org_id"]; ?>">
            ...
            <div class="control-group">
              ...
              <div class="controls">
                <textarea name="org_name" id="org_name" rows="2" class="span4"><?php echo $row[$i]["org_name"]; ?></textarea>
                <span class="help-inline">200 тэмдэгтээс хэтэрч болохгүй.</span> </div>
            </div>
            <div class="control-group">
              <label class="control-label">Регистрийн дугаар:</label>
              <div class="controls">
                <input type="text" name="register_number" id="register_number" class="span4"  value="<?php echo $row[$i]["register_number"]; ?>"/>
                <span class="help-inline">20 тэмдэгтээс хэтэрч болохгүй.</span> </div>
            </div>
            <div class="control-group">
              <label class="control-label">Байршлын хаяг :</label>
              <div class="controls">
                <textarea name="location_address" id="location_address" rows="2" class="span5"><?php echo $row[$i]["location_address"]; ?></textarea>
                <span class="help-inline">200 тэмдэгтээс хэтэрч болохгүй.</span> </div>
            </div>
            <div class="control-group">
              <label class="control-label">Утасны дугаар :</label>
              <div class="controls">
                <input type="text" name="tel_number" id="tel_number" class="span4" value="<?php echo $row[$i]["tel_number"]; ?>"/>
                <span class="help-inline">50 тэмдэгтээс хэтэрч болохгүй.</span> </div>
            </div>
            <div class="control-group">
              <label class="control-label">Факсын дугаар :</label>
              <div class="controls">
                <input type="text" name="fax_number" id="fax_number" class="span4" value="<?php echo $row[$i]["fax_number"]; ?>"/>
                <span class="help-inline">50 тэмдэгтээс хэтэрч болохгүй.</span> </div>
            </div>
            <div class="control-group">
              <label class="control-label">Имэйл:</label>
              <div class="controls">
                <textarea name="email_address" id="email_address" rows="3" class="span5"><?php echo $row[$i]["email_address"]; ?></textarea>
                <span class="help-inline">200 тэмдэгтээс хэтэрч болохгүй.</span> </div>
            </div>
            <div class="control-group">
              <label class="control-label">Вэб хаяг:</label>
              <div class="controls">
                <textarea name="web_address" id="web_address" rows="3" class="span5"><?php echo $row[$i]["web_address"]; ?></textarea>
                <span class="help-inline">200 тэмдэгтээс хэтэрч болохгүй.</span> </div>
            </div>
            <div class="control-group">
              <label class="control-label">Шуудангийн хаяг:</label>
              <div class="controls">
                <textarea name="postal_address" id="postal_address" rows="3" class="span5"><?php echo $row[$i]["postal_address"]; ?></textarea>
                <span class="help-inline">200 тэмдэгтээс хэтэрч болохгүй.</span> </div>
            </div>
            <div class="control-group">
            ...
            <input type="hidden" id="updateanimalorgbttn" name="updateanimalorgbttn" value="0"/>
