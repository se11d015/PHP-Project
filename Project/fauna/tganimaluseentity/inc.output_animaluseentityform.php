...
$valueQuery3 = "FROM ".$schemas.".tganimaluseentity tgcen, scadministrative.taaimagname taaim, scadministrative.tasoumname tasou ";
if ($outputtype == 1)
...
if (!empty($row)) {
    if ($outputtype == 1 || $outputtype == 2) {
        ...
        $buffer .= "Хуулийн этгээдийн төрөл: " . getdata($USE_ENTITY_TYPE, $row[$i]["entity_type"]) . "\r\n";
        $buffer .= "Нөөц ашиглагчийн нэр: " . $row[$i]["entity_name"] . "\r\n";
    ...
    if ($outputtype == 3) {
        ...
        $kml[] = '<value><![CDATA[' . getdata($USE_ENTITY_TYPE, $row[$i]["entity_type"]). ']]></value>';
        $kml[] = '</Data>';
        $kml[] = '<Data name="Нөөц ашиглагчийн нэр">';
        $kml[] = '<value><![CDATA[' . $row[$i]["entity_name"] . ']]></value>';
        $kml[] = '</Data>';
        $kml[] = '<Data name="Нөөц ашиглагчийн  регистрийн дугаар р">';
        ...
        header('Content-Disposition: attachment; filename="animaluseentitylocation.kml"');
        ob_clean();
        flush();
        echo $kmlOutput;
        exit;
