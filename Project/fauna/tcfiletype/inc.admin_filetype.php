...
if($sess_profile==1)
{
	if(isset($_POST["inserttypebttn"]))
	{
		if(isset($_POST["type_name"]))
		{
			...
			$result = $db->insert("".$schemas.".tcfiletype", $fields, $values);
	...
	if(isset($_POST["updatetypebttn"]))
	{
		...
		{	
			...
			$result = $db->update("".$schemas.".tcfiletype", $fields, $values, $wherevalues);
			if(! $result)
	...
	if(($action=="delete") && isset($_GET["typeid"]))
	{
		...
		$result = $db->delete("".$schemas.".tcfiletype", $wherevalues);
		if(! $result)
...
$selQuery = "SELECT tct.* FROM ".$schemas.".tcfiletype tct";
...
<form action="<?php echo $my_url; ?>" method="post" name="mainform" id="mainform">
  <div class="list-table">
    <table class="table table-bordered table-hover">
      <thead>
        <tr>
          <th colspan="3"><span class="title"><?php echo getdata($ITEM_TYPE, 121); ?></span></th>
        </tr>
      </thead>
      <thead>
        <tr>
      ...
      <tbody>
        <?php
			...
			$selQuery = "SELECT tct.* FROM ".$schemas.".tcfiletype tct ORDER BY tct.type_name ASC LIMIT ".$limit;
			$rows = $db->query($selQuery);
