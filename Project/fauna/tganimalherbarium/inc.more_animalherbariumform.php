...
<div class="more-table">
  <table class="table table-bordered table-striped table-hover">
    ...
    <tbody>
      <?php if(!empty($row[$i]["herbarium_filename"])) { ?>
      <tr>
        <td colspan="2"><div class="span3">
            <div class="thumbnail"><a href="<?php echo $row[$i]["herbarium_pathname"].$row[$i]["herbarium_filename"]; ?>" class="thumbnail" target="_blank" data-toggle="tooltip" data-placement="bottom" ><img src="<?php echo $row[$i]["herbarium_pathname"].$row[$i]["herbarium_filename"]; ?>" /> </a> </div>
          </div></td>
      </tr>
      <?php }?>
      <tr>
...
<div>
  <?php
	if ($sess_profile == 1) {
		?>
  <a class="btn btn-danger" href="<?php echo $my_url . $my_page . $search_url . $sort_url . "&action=edit&herbarium_id=" . $row[$i]["herbarium_id"]; ?>"><i class="icon-edit icon-white"></i>&nbsp;Засварлах</a>&nbsp;
  <?php
