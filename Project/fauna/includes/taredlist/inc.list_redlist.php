...
<div class="list-table">
  <table class="table table-bordered">
    <thead>
      <tr>
        <th><span class="title"><?php echo getdata($ITEM_TYPE, 20); ?></span></th>
      </tr>
    </thead>
  </table>
  <?php 
    require("includes/taredlist/inc.title_list_redlist.php");
  	require("includes/taredlist/inc.search_redlist.php");
	$notify ="<strong>Нийт $sum бичлэг байна.</strong>";
	show_notification("info", "", $notify);
	
		if(!empty($rows))
	{
		require("includes/taredlist/inc.title_redlist.php");
  ...
  <table class="table table-bordered table-striped table-hover">
    <thead>
      <tr>
        ...
        <th class="span4"><a href="<?php echo $my_url."&sort=6&sorttype=".$sorttype.$search_url.$list_url; ?>" style="color:#FFFFFF"><i class="icon-tag icon-white"></i> Олон улсын үнэлгээ </a></th>
        <th class="span4"><a href="<?php echo $my_url."&sort=7&sorttype=".$sorttype.$search_url.$list_url; ?>" style="color:#FFFFFF"><i class="icon-tag icon-white"></i> Бүс нутгийн үнэлгээ </a></th>
        <th class="span2"></th>
      </tr>
    </thead>
    <tbody>
      ...
      <tr>
        ...
        <td align="center"><a href="<?php echo $my_url.$my_page.$search_url.$list_url.$sort_url."&action=more&redlist_id=".$rows[$i]["redlist_id"]; ?>" title="Дэлгэрэнгүй харах"><i class="icon-list"></i></a> 
      </tr>
      <?php 
		}	
		?>
  ...
  <?php 
    ...
    <tbody>
      <?php
		$startQuery = "SELECT";
		$limit = $count." OFFSET ".($page-1)*$count;
		$valueQuery = "tarl.*, tcrs.name_mn global_name_mn, tcrs1.name_mn regional_name_mn, tacn.class_name, tafn.family_name, taon.order_name,  tapl.species_name  FROM ".$schemas.".taredlist tarl,".$schemas.".takingdomname takn,".$schemas.".taphylumname tapn,".$schemas.".taclassname tacn,".$schemas.".tafamilyname tafn,".$schemas.".taordername taon,".$schemas.".tagenusname tagn,".$schemas.".taanimalname tapl, ".$schemas.".tcredliststatus tcrs, ".$schemas.".tcredliststatus tcrs1";
		$whereQuery = "WHERE tarl.redlist_id = tarl.redlist_id AND tarl.species_code = tapl.species_code AND tapl.genus_code = tagn.genus_code AND tagn.family_code = tafn.family_code AND tafn.order_code = taon.order_code AND taon.class_code=tacn.class_code AND tacn.phylum_code=tapn.phylum_code  AND tapn.kingdom_code=takn.kingdom_code AND tarl.global_code = tcrs.status_code AND tarl.regional_code = tcrs1.status_code";
