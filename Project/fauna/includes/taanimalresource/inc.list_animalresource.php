...
if (isset($_POST["searchresourcebttn"])) 
{

	$searchQuery = "";
	$search_url = "";
	
	$evaluated_date = (isset($_POST["evaluated_date"])) ? (int) $_POST["evaluated_date"] : 0;
...
if (isset($_GET["sort"])) {
	...
	}elseif ($_GET["sort"]==15)
	{
		...
		$sortQuery .= "taar.evaluated_date";
	} else {
    $sortQuery .= " taar.resource_id";
	}
} else {
	$sortQuery .= " taar.resource_id";
}
...
$startQuery = "SELECT";
$valueQuery = "COUNT(*) AS num_count FROM " . $schemas . ".takingdomname takn," . $schemas . ".taphylumname tapn," . $schemas . ".taclassname tacn," . $schemas . ".tafamilyname tafn," . $schemas . ".taordername taon," . $schemas . ".tagenusname tagn," . $schemas . ".taanimalname tapl," . $schemas . ".taanimalresource taar ";
$whereQuery = "WHERE tapl.species_code = taar.species_code AND tapl.genus_code = tagn.genus_code AND tagn.family_code = tafn.family_code AND tafn.order_code = taon.order_code AND taon.class_code=tacn.class_code AND tacn.phylum_code=tapn.phylum_code  AND tapn.kingdom_code=takn.kingdom_code  AND taar.resource_id=taar.resource_id";
...
<div class="list-table">
  <table class="table table-bordered">
    <thead>
      <tr>
        <th><span class="title"><?php echo getdata($ITEM_TYPE, 11); ?></span></th>
      </tr>
    </thead>
  </table>
  <?php 
  require("includes/taanimalresource/inc.search_animalresource.php");
  ...
  <table class="table table-bordered table-striped table-hover">
    <thead>
      <tr>
        <th >№</th>
        <th class="span4"><a href="<?php echo $my_url."&sort=1&sorttype=".$sorttype.$search_url.$list_url; ?>" style="color :#FFFFFF"><i class="icon-tag icon-white"></i> Ангиин монгол нэр</a></th>
        ...
        <th class="span4"><a href="<?php echo $my_url . "&sort=6&sorttype=" . $sorttype . $search_url.$list_url; ?>" style="color :#FFFFFF"><i class="icon-tag icon-white"></i> Судалгаа хийсэн огноо</a></th>
        <th class="span2"></th>
      </tr>
    </thead>
    <tbody>
      <?php
		...
		$valueQuery = "taar.*, tacn.class_name_mn, tafn.family_name_mn, taon.order_name_mn, tagn.genus_name, tagn.genus_name_mn, tapl.species_name_mn, tapl.species_name FROM " . $schemas . ".takingdomname takn," . $schemas . ".taphylumname tapn," . $schemas . ".taclassname tacn," . $schemas . ".tafamilyname tafn," . $schemas . ".taordername taon," . $schemas . ".tagenusname tagn," . $schemas . ".taanimalname tapl," . $schemas . ".taanimalresource taar ";
		$whereQuery = "WHERE tapl.species_code = taar.species_code AND tapl.genus_code = tagn.genus_code AND tagn.family_code = tafn.family_code AND tafn.order_code = taon.order_code AND taon.class_code=tacn.class_code AND tacn.phylum_code=tapn.phylum_code  AND tapn.kingdom_code=takn.kingdom_code  AND taar.resource_id=taar.resource_id";
      ...
      <tr>
        ...
        <td align="center"><a href="<?php echo $my_url . $my_page . $search_url .$list_url. $sort_url . "&action=more&resource_id=" . $rows[$i]["resource_id"]; ?>" title="Дэлгэрэнгүй харах"><i class="icon-list"></i></a></td>
      </tr>
      <?php
        }
        ?>
    ...
    <thead>
      <tr>
        <th >№</th>
        <th class="span4"><a href="<?php echo $my_url."&sort=11&sorttype=".$sorttype.$search_url.$list_url; ?>" style="color :#FFFFFF"><i class="icon-tag icon-white"></i> Ангиин латин нэр</a></th>
    ...
    <tbody>
      <?php
		$startQuery = "SELECT";	  
		$limit = $count . " OFFSET " . ($page - 1) * $count;
		$valueQuery = "taar.*, tacn.class_name, tafn.family_name, taon.order_name, tagn.genus_name, tagn.genus_name, tapl.species_name, tapl.species_name FROM " . $schemas . ".takingdomname takn," . $schemas . ".taphylumname tapn," . $schemas . ".taclassname tacn," . $schemas . ".tafamilyname tafn," . $schemas . ".taordername taon," . $schemas . ".tagenusname tagn," . $schemas . ".taanimalname tapl," . $schemas . ".taanimalresource taar ";
		$whereQuery = "WHERE tapl.species_code = taar.species_code AND tapl.genus_code = tagn.genus_code AND tagn.family_code = tafn.family_code AND tafn.order_code = taon.order_code AND taon.class_code=tacn.class_code AND tacn.phylum_code=tapn.phylum_code  AND tapn.kingdom_code=takn.kingdom_code  AND taar.resource_id=taar.resource_id";
