  ...
  <div class="row">
    <form class="form-horizontal" action="<?php echo $my_url; ?>" method="post" name="mainform" id="mainform">
      ...
      <div class="span6 search-control">
        ...
        <div class="control-group info">
          <label class="control-label">Олон улсын үнэлгээ:</label>
          <div class="controls">
            <?php
				$selQuery = "SELECT tcr.status_code, tcr.name_mn FROM ".$schemas.".tcredliststatus tcr ORDER BY tcr.name_mn";
				$row = $db->query($selQuery);
				echo seldatadb("global_code", "span3", $row, "status_code", "name_mn", $global_code, "", "Бүх үнэлгээ");
				?>
          </div>
        </div>
        <div class="control-group info">
          <label class="control-label">Бүс нутгийн үнэлгээ:</label>
          <div class="controls">
            <?php
				$selQuery = "SELECT tcr.status_id, tcr.status_code, tcr.name_mn FROM ".$schemas.".tcredliststatus tcr ORDER BY tcr.name_mn";
				$row = $db->query($selQuery);
				echo seldatadb("regional_code", "span3", $row, "status_code", "name_mn", $regional_code, "", "Бүх үнэлгээ");
				?>
          </div>
        </div>
        <div class="control-group info">
          <div class="controls">
            <button type="submit" class="btn btn-danger" name="searchredlistbttn"><i class="icon-search icon-white"></i>&nbsp;Хайх</button>
          </div>
        </div>
      </div>
    </form>
