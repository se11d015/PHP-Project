...
if ($db->isGroupRole($schemas, $sess_profile, $sess_user_id, 8, 2)) 
{
	...
	$startQuery = "SELECT";
	$valueQuery = "tgcpe.*, taupa.*, tgcen.entity_name, tapl.species_code, takn.kingdom_name, takn.kingdom_name_mn, takn.kingdom_name_en, takn.kingdom_name_ru, tapn.phylum_name, tapn.phylum_name_mn, tapn.phylum_name_en, tapn.phylum_name_ru, tacn.class_name, tacn.class_name_mn, tacn.class_name_en, tacn.class_name_ru, taon.order_name, taon.order_name_mn, taon.order_name_en, taon.order_name_ru, tafn.family_name, tafn.family_name_mn, tafn.family_name_en, tafn.family_name_ru, tagn.genus_name, tagn.genus_name_mn, tagn.genus_name_en, tagn.genus_name_ru, tapl.species_name, tapl.species_name_mn,  tapl.species_name_en, tapl.species_name_ru FROM " . $schemas . ".takingdomname takn," . $schemas . ".taphylumname tapn," . $schemas . ".taclassname tacn," . $schemas . ".tafamilyname tafn," . $schemas . ".taordername taon," . $schemas . ".tagenusname tagn," . $schemas . ".taanimalname tapl, " . $schemas . ".taanimalusepayment taupa, ".$schemas.".tganimaluseentity tgcen, ".$schemas.".taanimalusepermission tgcpe";
	
	
	if ($sess_profile == 1)
		$whereQuery = "WHERE tgcpe.permission_id = taupa.permission_id  AND tgcpe.entity_name = tgcen.entity_id  AND taupa.species_name = tapl.species_code AND tapl.genus_code = tagn.genus_code AND tagn.family_code = tafn.family_code AND tafn.order_code = taon.order_code AND taon.class_code=tacn.class_code AND tacn.phylum_code=tapn.phylum_code  AND tapn.kingdom_code=takn.kingdom_code AND taupa.species_id = " . $species_id;
	else
		$whereQuery = "WHERE tgcpe.permission_id = taupa.permission_id  AND tgcpe.entity_name = tgcen.entity_id  AND taupa.species_name = tapl.species_code AND tapl.genus_code = tagn.genus_code AND tagn.family_code = tafn.family_code AND tafn.order_code = taon.order_code AND taon.class_code=tacn.class_code AND tacn.phylum_code=tapn.phylum_code  AND tapn.kingdom_code=takn.kingdom_code AND taupa.species_id = " . $species_id . " AND taupa.user_id = " . $sess_user_id;
...
function updatesubmitform()
{
	if (document.getElementById("species_code").value==""){
		...
		alert("Амьтны хэмжээг оруулна уу");
	} else {
      ...
      <tr>
        <td><form class="form-horizontal" action="<?php echo $my_url . $search_url . $sort_url; ?>" enctype="multipart/form-data" method="post" name="mainform" id="mainform">
            ...
            <div class="control-group">
              ...
              <div class="controls"><?php echo $row[$i]["approved_org"]; ?> </div>
            </div>
            <div class="control-group">
            ...
            <div class="control-group">
              <label class="control-label">Амьтны хэмжээ:</label>
              <div class="controls">
