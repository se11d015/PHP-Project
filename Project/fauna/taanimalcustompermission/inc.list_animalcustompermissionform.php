...
$startQuery = "SELECT";
$valueQuery = " COUNT(*) AS num_count   FROM ".$schemas.".taanimalcustompermission tgcpe, ".$schemas.".tganimalcustomentity tgcen";
...
require("inc.search_animalcustompermissionform.php");
...
<div class="list-table">
  <table class="table table-bordered table-hover">
    <thead>
      <tr>
        <th colspan="9"><span class="title"><?php echo getdata($ITEM_TYPE, 61); ?></span></th>
      </tr>
    </thead>
    <thead>
      <tr>
        <td>№</td>
        <th class="span4"><a href="<?php echo $my_url."&sort=1&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="icon-tag icon-white"></i> Экспортлогч, импортлогчийн нэр</a></th>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?php echo getdata($CUSTOM_PERMISSION_TYPE, $rows[$i]["permission_type"]); ?></td>
        ...
        <td><a href="<?php echo $my_url.$my_page.$search_url.$sort_url."&action=more&permission_id=".$rows[$i]["permission_id"]; ?>" title="Дэлгэрэнгүй харах"><i class="icon-list"></i></a>
          <?php
			...
			{
			?>
          <a href="<?php echo $my_url.$my_page.$search_url.$sort_url."&action=edit&permission_id=".$rows[$i]["permission_id"]; ?>"><i class="icon-edit"></i></a> <a href="<?php echo $my_url.$search_url.$sort_url."&action=delete&permission_id=".$rows[$i]["permission_id"]; ?>" title="Устгах" onClick="return confirm('Та устгахдаа итгэлтэй байна уу?');"> <i class="icon-trash"></i></a> <a href="<?php echo $my_url.$my_page.$search_url.$sort_url."&action=customadd&permission_id=".$rows[$i]["permission_id"]; ?>" title="Амьтны жагсаалт нэмэх"> <i class="icon-plus"></i></a>
          <?php 
