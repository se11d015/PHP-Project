...
if ($db->isGroupRole($schemas, $sess_profile, $sess_user_id, 6, 2)) 
{
	...
	$startQuery = "SELECT";
	$valueQuery = "tgcen.*, st_x(tgcen.geom) as geomx, st_y(tgcen.geom) as geomy,  taaim.aimag_name_mn, tasou.soum_name_mn  FROM ".$schemas.".tganimalcustomentity tgcen, scadministrative.taaimagname taaim, scadministrative.tasoumname tasou ";
	
	if ($sess_profile == 1)
		$whereQuery = "WHERE tgcen.entity_id = tgcen.entity_id AND taaim.aimag_code=tgcen.aimag_name  AND tasou.soum_code=tgcen.soum_name AND tgcen.entity_id = " . $entity_id;
	else
		$whereQuery = "WHERE tgcen.entity_id = " . $entity_id . " AND tgcen.entity_id = tgcen.entity_id AND taaim.aimag_code=tgcen.aimag_name  AND tasou.soum_code=tgcen.soum_name AND tgcen.user_id = " . $sess_user_id;
...
function updatesubmitform()
{
    ...
    alert("Экспортлогч, импортлогчийн нэр оруулна уу");
			...
			document.getElementById("updateanimalcustomentitybttn").value = "1";
...
<div class="add-table">
  <table class="table table-bordered table-condensed">
    <thead>
      <tr>
        <th><?php echo getdata($ITEM_TYPE, 60); ?> засах хэсэг</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><form class="form-horizontal" action="<?php echo $my_url . $search_url . $sort_url; ?>" enctype="multipart/form-data" method="post" name="mainform" id="mainform">
            ...
            <div class="control-group">
              ...
              <div class="controls">
                <?php
					echo seldata("entity_type", "span2", $CUSTOM_ENTITY_TYPE, $row[$i]["entity_type"]);
					?>
                <span class="help-inline">Жагсаалтаас аль нэгийг нь сонгоно.</span> </div>
            </div>
            <div class="control-group">
              ...
              <div class="controls">
                <textarea name="entity_name" id="entity_name" rows="3" class="span5"><?php echo C; ?></textarea>
            ...
            <input type="hidden" id="updateanimalcustomentitybttn" name="updateanimalcustomentitybttn" value="0"/>
