...
$startQuery = "SELECT";
$valueQuery1 = "tgf.gid, taf.reforest_year, vs.aimag_name_mn, vs.aimag_name_en, vs.soum_name_mn, vs.soum_name_en, tgf.planted_area";
if ($outputtype == 1)
...
$valueQuery3 = "FROM ".$schemas.".tgplantedforest tgf, ".$schemas.".tareforestation taf, scadministrative.vasoumname vs";
...
if (!empty($row)) {
    if ($outputtype == 1 || $outputtype == 3) {
        $buffer = "";
        $buffer .= "№: ".$row[$i]["gid"]."\r\n";
        $buffer .= _p("GisSub3Column3").": ".$row[$i]["reforest_year"]."\r\n";
        $buffer .= _p("GisSub3Column1").": ".$row[$i]["aimag_name_$language_name"]."\r\n";
        $buffer .= _p("GisSub3Column2").": ".$row[$i]["soum_name_$language_name"]."\r\n";
        $buffer .= _p("GisSub3Column4").": ".$row[$i]["planted_area"]."\r\n";;
        $buffer .= _p("GeometryText").": ".$row[$i]["geomtext"]."\r\n";

        $filename = "plantedforest.txt";
    ...
    if ($outputtype == 2) {
        ...
        $kml[] = '<value><![CDATA[' . $row[$i]["planted_area"] . ']]></value>';
        $kml[] = '</Data>';
        $kml[] = '</ExtendedData>';
