<div class="content-wrapper">
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col">
        <h1> <?php echo _p("GisSubTitle3"); ?> </h1>
		  <?php
			if ($db->isGroupRole($schemas, $sess_profile, $sess_user_id, 13, 1)) 
			{
				...
				require("tgplantedforest/inc.admin_export_plantedforest.php");
				
				if ($sess_profile==1 || $db->isGroupRole($schemas, $sess_profile, $sess_user_id, 13, 2)) 
				{
					require("modules/generategeom.class.php");

					if (isset($_POST["insertplantedforestgeombttn"]) && (int) $_POST["insertplantedforestgeombttn"] == 1) 
					{
						if (isset($_POST["reforest_id"]))
						{
							...
							$fields = array("reforest_id", "planted_area", "geom", "user_id");
							
							$checkvalues = array($reforest_id, $planted_area);
							...
							$result = $db->insert("".$schemas.".tgplantedforest", $fields, $values);
							if(! $result)
					...
					if (isset($_POST["updateplantedforestgeombttn"]) && (int) $_POST["updateplantedforestgeombttn"] == 1) 
					{
			     
						if (isset($_POST["reforest_id"]) && isset($_POST["gid"]))
						{
							...
							if($geomtype==6)
							{
								$fields = array("reforest_id", "planted_area", "user_id");
							...
							$result = $db->update("".$schemas.".tgplantedforest", $fields, $values, $wherevalues);
							if (!$result) {
					...
					if (($action == "delete") && isset($_GET["gid"])) 
					{
						...
						$result = $db->delete("".$schemas.".tgplantedforest", $wherevalues);
						if (!$result)
				...
				{
					require("tgplantedforest/inc.select_admin_reforestation.php");
				}elseif ($action=="export")
				{
					require("tgplantedforest/inc.export_plantedforest.php");
				} elseif ($action == "more")
				{
					require("tgplantedforest/inc.more_plantedforest.php");
				} elseif ($action == "outputgeom")
				{
					require("tgplantedforest/inc.output_plantedforest.php");					
				} else
				{
					require("tgplantedforest/inc.list_plantedforest.php");
				}
			} else {
