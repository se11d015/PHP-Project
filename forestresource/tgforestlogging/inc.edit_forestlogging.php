...
if ($sess_profile==1 || $db->isGroupRole($schemas, $sess_profile, $sess_user_id, 14, 2)) 
{
	...
	$startQuery = "SELECT";
	$valueQuery = "tgf.*, taf.utilization_year, vs.aimag_name_mn, vs.aimag_name_en, vs.soum_name_mn, vs.soum_name_en 
	...
	if($sess_profile==1)
		...
		$whereQuery = "WHERE tgf.utilization_id = taf.utilization_id AND taf.soum_code = vs.soum_code AND tgf.gid = ".$gid." AND tgf.user_id = ".$sess_user_id;
...
{
    document.getElementById("updateforestlogginggeombttn").value = "1";
...
<div class="table-responsive">
  <table class="table table-bordered table-striped">
    <thead>
      <tr>
        <th><?php echo _p("EditText3")." "._p("GisSubTitle4")." "._p("EditText4"); ?></th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><div class="alert alert-warning" role="alert"><?php echo _p("AlertText"); ?></div>
          <form class="form" action="<?php echo $my_url.$my_page.$search_url.$sort_url; ?>" enctype="multipart/form-data" method="post" name="mainform" id="mainform">
			    ...
			    <table>
				 <tbody>
					...
					<tr>
					  ...
					  <td>
					    <div class="form-check form-check-inline">
						  <input class="form-check-input" type="radio" name="logging_type" id="logging_type" value="1" <?php if($row[$i]["logging_type"]==1) echo "checked"; ?>>
						  <label class="form-check-label" for="inlineCheckbox1"><?php echo _p("GisSub4MoreColumn4");?></label>
						</div>
						<div class="form-check form-check-inline">
						  <input class="form-check-input" type="radio" name="logging_type" id="logging_type" value="2" <?php if($row[$i]["logging_type"]==2) echo "checked"; ?>>
						  <label class="form-check-label" for="inlineCheckbox2"><?php echo _p("GisSub4MoreColumn5");?></label>
						</div>
						<div class="form-check form-check-inline">
						  <input class="form-check-input" type="radio" name="logging_type" id="logging_type" value="3" <?php if($row[$i]["logging_type"]==3) echo "checked"; ?>>
					...
					<tr>
					  ...
					  <td><input type="text" class="form-control" name="logging_area" id="logging_area" value="<?php echo $row[$i]["logging_area"]; ?>"/></td>
					  ...
					  <tr>
						...
						<td><?php echo _p("GisSub4Unit3");?></td>
						<td><input type="text" disabled class="form-control" name="logged_total" id="logged_total" value="<?php echo $row[$i]["logged_total"]; ?>"/></td>
						<td><span class="form-text text-muted"><?php echo _p("AlertTextReal10"); ?></span></td>
					  </tr>
					<tr>
					  <th>4.</th>
            ...
            <input type="hidden" id="updateforestlogginggeombttn" name="updateforestlogginggeombttn" value="0"/>
