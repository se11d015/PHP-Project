...
if (isset($_POST["searchforestfirebttn"])) 
{
	...
	$sfire_year = (isset($_POST["sfire_year"])) ? (int) $_POST["sfire_year"] : 0;		
...
if (isset($_GET["sort"])) 
{
	...
	{
        $sortQuery .= " taf.total_burnt_area";
    } else if ($_GET["sort"] == 5) 
	{
        $sortQuery .= " taf.total_damage_cost";		
    } else if ($_GET["sort"] == 6) 
	{
        $sortQuery .= " taf.total_damp_cost";				
    } else
	{
        $sortQuery .= " taf.fire_year";
    }
} else 
{
    $sortQuery .= " taf.fire_id";
}
...
$startQuery = "SELECT";
$valueQuery = "COUNT(*) AS num_count FROM ".$schemas.".taforestfire taf, scadministrative.vasoumname vs";
...
require("taforestfire/inc.search_forestfire.php");	
...
<div class="table-responsive">
  <table id="forestresource_datatables" class="table table-bordered table-hover" title_name="<?php echo _p("ResourceSubTitle3"); ?>" file_name="forestfire" column_name="0, 1, 2, 3, 4, 5, 6" language_name="<?php echo $language_name;?>" page_count="<?php echo $count;?>">
    <thead>
      <tr>
      ...
      <tr>
        <th>№</th>
        <th><a href="<?php echo $my_url."&sort=1&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="fa fa-sort"></i> <?php echo _p("ResourceSub3Column3"); ?></a></th>		
        <th><a href="<?php echo $my_url."&sort=2&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="fa fa-sort"></i> <?php echo _p("ResourceSub3Column1"); ?></a></th>
        <th><a href="<?php echo $my_url."&sort=3&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="fa fa-sort"></i> <?php echo _p("ResourceSub3Column2"); ?></a></th>
        <th><a href="<?php echo $my_url."&sort=4&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="fa fa-sort"></i> <?php echo _p("ResourceSub3Column4"); ?></a></th>
        <th><a href="<?php echo $my_url."&sort=5&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="fa fa-sort"></i> <?php echo _p("ResourceSub3Column5"); ?></a></th>
        <th><a href="<?php echo $my_url."&sort=6&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="fa fa-sort"></i> <?php echo _p("ResourceSub3Column6"); ?></a></th>		
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?php echo $rows[$i]["fire_year"]; ?></td>
        <td><?php echo $rows[$i]["aimag_name_$language_name"]; ?></td>
        <td><?php echo $rows[$i]["soum_name_$language_name"]; ?></td>
        <td><?php echo $rows[$i]["total_burnt_area"]; ?></td>
        <td><?php echo $rows[$i]["total_damage_cost"]; ?></td>
        <td><?php echo $rows[$i]["total_damp_cost"]; ?></td>
        <td><a href="<?php echo $my_url.$my_page.$search_url.$sort_url."&action=more&fire_id=".$rows[$i]["fire_id"]; ?>" title="<?php echo _p("MoreTitle"); ?>"><i class="fa fa-list-alt"></i></a>
          <?php 
			if($sess_profile==1)
			{ 
			?>
          <a href="<?php echo $my_url.$my_page.$search_url.$sort_url."&action=edit&fire_id=".$rows[$i]["fire_id"]; ?>" title="<?php echo _p("EditTitle"); ?>"><i class="fa fa-pencil"></i></a> <a href="<?php echo $my_url.$search_url.$sort_url."&action=delete&fire_id=".$rows[$i]["fire_id"]; ?>" title="<?php echo _p("DeleteTitle"); ?>" onClick="return confirm('<?php echo _p("DeleteText3"); ?>');"><i class="fa fa-trash"></i></a>
          <?php
