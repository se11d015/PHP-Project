...
$startQuery = "SELECT";
$valueQuery1 = "tgf.gid, taf.owner_year, vs.aimag_name_mn, vs.aimag_name_en, vs.soum_name_mn, vs.soum_name_en, tgf.owner_area";
if ($outputtype == 1)
...
$valueQuery3 = "FROM ".$schemas.".tgownerforest tgf, ".$schemas.".taownerforest taf, scadministrative.vasoumname vs";
...
if (!empty($row)) {
    if ($outputtype == 1 || $outputtype == 3) {
        $buffer = "";
        $buffer .= "№: ".$row[$i]["gid"]."\r\n";
        $buffer .= _p("GisSub5Column3").": ".$row[$i]["owner_year"]."\r\n";
        $buffer .= _p("GisSub5Column1").": ".$row[$i]["aimag_name_$language_name"]."\r\n";
        $buffer .= _p("GisSub5Column2").": ".$row[$i]["soum_name_$language_name"]."\r\n";
        $buffer .= _p("GisSub5Column5").": ".$row[$i]["owner_area"]."\r\n";;
        $buffer .= _p("GeometryText").": ".$row[$i]["geomtext"]."\r\n";

        $filename = "ownerforest.txt";
    ...
    if ($outputtype == 2) {
        ...
        $kml[] = '<value><![CDATA[' . $row[$i]["owner_year"] . ']]></value>';
        $kml[] = '</Data>';
        ...
        $kml[] = '<value><![CDATA[' . $row[$i]["owner_area"] . ']]></value>';
        $kml[] = '</Data>';
        $kml[] = '</ExtendedData>';
