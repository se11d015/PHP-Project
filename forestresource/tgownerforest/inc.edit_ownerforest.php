...
if ($sess_profile==1 || $db->isGroupRole($schemas, $sess_profile, $sess_user_id, 15, 2)) 
{
	...
	$startQuery = "SELECT";
	$valueQuery = "tgf.*, taf.owner_year, vs.aimag_name_mn, vs.aimag_name_en, vs.soum_name_mn, vs.soum_name_en 
	FROM ".$schemas.".tgownerforest tgf, ".$schemas.".taownerforest taf, scadministrative.vasoumname vs";
	
	if($sess_profile==1)
		$whereQuery = "WHERE tgf.owner_id = taf.owner_id AND taf.soum_code = vs.soum_code AND tgf.gid = ".$gid;
	else
		$whereQuery = "WHERE tgf.owner_id = taf.owner_id AND taf.soum_code = vs.soum_code AND tgf.gid = ".$gid." AND tgf.user_id = ".$sess_user_id;
...
{
    document.getElementById("updateownerforestgeombttn").value = "1";
...
<div class="table-responsive">
  <table class="table table-bordered table-striped">
    <thead>
      <tr>
        <th><?php echo _p("EditText3")." "._p("GisSubTitle5")." "._p("EditText4"); ?></th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><div class="alert alert-warning" role="alert"><?php echo _p("AlertText"); ?></div>
          <form class="form" action="<?php echo $my_url.$my_page.$search_url.$sort_url; ?>" enctype="multipart/form-data" method="post" name="mainform" id="mainform">
            ...
            <div class="form-group row">
              <label class="col-md-3 col-form-label"><?php echo _p("GisSub5Column3"); ?>:</label>
              <div class="col-md-4">
                <input type="text" readonly class="form-control-plaintext" value="<?php echo $row[$i]["owner_year"]; ?>"/>
              </div>
            </div>				
            <div class="form-group row">
              <label class="col-md-3 col-form-label"><?php echo _p("GisSub5Column1"); ?>:</label>
              <div class="col-md-4">
                <input type="text" readonly class="form-control-plaintext" value="<?php echo $row[$i]["aimag_name_$language_name"]; ?>"/>
              </div>
            </div>
			 <div class="form-group row">
              <label class="col-md-3 col-form-label"><?php echo _p("GisSub5Column2"); ?>:</label>
              <div class="col-md-4">
			  ...
			  <div class="form-group row col-md-12">
			    <table>
				 <tbody>
				    <tr>
					  <th style="width: 5%">№</th>
					  <th style="width: 35%"><?php echo _p("GisSub5ColumnText1");?></th>
					  <th style="width: 10%"><?php echo _p("GisSub5ColumnText2");?></th>
					  <th><?php echo _p("GisSub5ColumnText3");?></th>
					  <th><?php echo _p("Description");?></th>
				    </tr>
					<tr>
					  <th>1.</th>
					  <td><?php echo _p("GisSub5Column4");?></td>
					  <td><?php echo _p("GisSub5Unit1");?></td>
					  <td>
					    <div class="form-check form-check-inline">
						  <input class="form-check-input" type="radio" name="owner_type" id="owner_type" value="1" <?php if($row[$i]["owner_type"]==1) echo "checked"; ?>>
						  <label class="form-check-label" for="inlineCheckbox1"><?php echo _p("GisSub5MoreColumn1");?></label>
						</div>
						<div class="form-check form-check-inline">
						  <input class="form-check-input" type="radio" name="owner_type" id="owner_type" value="2" <?php if($row[$i]["owner_type"]==2) echo "checked"; ?>>
						  <label class="form-check-label" for="inlineCheckbox2"><?php echo _p("GisSub5MoreColumn2");?></label>
						</div>
						<div class="form-check form-check-inline">
						  <input class="form-check-input" type="radio" name="owner_type" id="owner_type" value="3" <?php if($row[$i]["owner_type"]==3) echo "checked"; ?>>
						  <label class="form-check-label" for="inlineCheckbox3"><?php echo _p("GisSub5MoreColumn3");?></label>
						</div></td>
					...
					<tr>
					  <th>2.</th>
					  <td><?php echo _p("GisSub5Column4");?></td>
					  <td><?php echo _p("GisSub5Unit2");?></td>
					  <td><input type="text" class="form-control" name="owner_area" id="owner_area" value="<?php echo $row[$i]["owner_area"]; ?>"/></td>
					  <td><span class="form-text text-muted"><?php echo _p("AlertTextReal11"); ?></span></td>
					</tr>
					  <tr>
						<th>3.</th>
						<td><?php echo _p("GisSub5Column6");?></td>
						<td><?php echo _p("GisSub5Unit1");?></td>
					  ...
					  <tr>
						<th>4.</th>
						<td><?php echo _p("GisSub5Column7");?></td>
						<td><?php echo _p("GisSub5Unit1");?></td>
						<td><textarea class="form-control" name="owner_act" id="owner_act" rows="3"><?php echo $row[$i]["owner_act"]; ?></textarea></td>
						<td><span class="form-text text-muted"><?php echo _p("AlertTextCharAll"); ?></span></td>
					  </tr>
					  <tr>
						<th>5.</th>
						<td><?php echo _p("GisSub5Column8");?></td>
						<td><?php echo _p("GisSub5Unit1");?></td>
						<td><input type="file" class="form-control-file" name="owner_plan" id="owner_plan" /><input type="hidden" id="owner_plan" name="owner_plan" value="<?php echo $row[$i]["owner_plan"]; ?>"/></td>
						<td><span class="form-text text-muted"><?php echo _p("BrowseFileText1"); ?><?php if(!empty($row[$i]["owner_plan"])) { ?> <br> <a href="<?php echo $row[$i]["owner_plan"]; ?>" target="_blank"><?php echo $row[$i]["owner_plan"]; ?></a><?php } ?></span></td>
					  </tr>
					<tr>
					  <th>6.</th>
            ...
            <input type="hidden" id="updateownerforestgeombttn" name="updateownerforestgeombttn" value="0"/>
