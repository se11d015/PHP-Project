<div class="content-wrapper">
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col">
        <h1> <?php echo _p("GisSubTitle5"); ?> </h1>
		  <?php
			if ($db->isGroupRole($schemas, $sess_profile, $sess_user_id, 15, 1)) 
			{
				...
				require("tgownerforest/inc.admin_export_ownerforest.php");
				...
				if ($sess_profile==1 || $db->isGroupRole($schemas, $sess_profile, $sess_user_id, 15, 2)) 
				{
					...
					if (isset($_POST["insertownerforestgeombttn"]) && (int) $_POST["insertownerforestgeombttn"] == 1) 
					{
						if (isset($_POST["owner_id"]) && isset($_POST["owner_type"]))
						{
							$owner_id = (int) $_POST["owner_id"];
							$owner_type = (int) $_POST["owner_type"];
								
							$owner_area = floatval(str_replace(",",".",$_POST["owner_area"]));
							...
							if (is_uploaded_file($_FILES['owner_plan']['tmp_name']))
							{
								$today = date('Y-m-d');
								...
								$path = $path."/".$_MY_CONF["MANAGEMENT_PATH"]."/";
								if (!is_dir($path))
								{
									mkdir($path, 0775);
									chmod($path, 0775);
								...
								$uploader = new pdf_upload();
								$uploader->first_values('','','MB','20') ;
								$uploader->uploader_set($_FILES['owner_plan'], date("Y", strtotime($today)).date("m", strtotime($today)).date("d", strtotime ($today)), $path, $PDF_TYPES);
									
								if ($uploader->uploaded)
								{
									$owner_file = $uploader->uploaded_files[0];
									$owner_plan = $path.$owner_file;
								} else 
								{
							...
							$fields = array("owner_id", "owner_type", "owner_area", "owner_location", "owner_act", "owner_plan", "geom", "user_id");
							
							$checkvalues = array($owner_id, $owner_type, $owner_area, $_POST["owner_location"], $_POST["owner_act"], $owner_plan);
							...
							$result = $db->insert("".$schemas.".tgownerforest", $fields, $values);
							if(! $result) {
					...
					if (isset($_POST["updateownerforestgeombttn"]) && (int) $_POST["updateownerforestgeombttn"] == 1) 
					{
			     
						if (isset($_POST["owner_id"]) && isset($_POST["owner_type"]) && isset($_POST["gid"]))
						{
							...
							$owner_file = "";
							$owner_plan = $owner_oldplan = "";
							$owner_plan_changed = 0;
							
							if(isset($_POST["owner_plan"]) && !empty($_POST["owner_plan"]))
								$owner_oldplan = $owner_plan = $_POST["owner_plan"];
							...
							if($geomtype==6)
							{
								$fields = array("owner_id", "owner_type", "owner_area", "owner_location", "owner_act", "owner_plan", "user_id");
							...
							$result = $db->update("".$schemas.".tgownerforest", $fields, $values, $wherevalues);
							if (!$result) {
								show_notification("error", _p("EditText1"), "");
							} else {
								show_notification("success", _p("EditText2"), "");
								if($owner_plan_changed==1 && !empty($owner_oldplan))
									unlink($owner_oldplan);	
							}
						}
					}
					...
					if (($action == "delete") && isset($_GET["gid"])) 
					{
						...
						$selQuery = "SELECT owner_plan FROM ".$schemas.".tgownerforest WHERE ".$wherevalues;
						$rowfile = $db->query($selQuery);
						
						$result = $db->delete("".$schemas.".tgownerforest", $wherevalues);
						if (!$result) {
							show_notification("error", _p("DeleteText1"), "");
						} else {
							show_notification("success", _p("DeleteText2"),"");
							if (!empty($rowfile[0]["owner_plan"])) {
								unlink($rowfile[0]["owner_plan"]);
							}
						}
					}
				...
				{
					require("tgownerforest/inc.select_admin_ownerforest.php");
				}elseif ($action=="export")
				{
					require("tgownerforest/inc.export_ownerforest.php");
				} elseif ($action == "more")
				{
					require("tgownerforest/inc.more_ownerforest.php");
				} elseif ($action == "outputgeom")
				{
					require("tgownerforest/inc.output_ownerforest.php");					
				} else
				{
					require("tgownerforest/inc.list_ownerforest.php");
				}
			} else {
