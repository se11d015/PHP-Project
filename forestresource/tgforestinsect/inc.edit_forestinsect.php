...
if ($sess_profile==1 || $db->isGroupRole($schemas, $sess_profile, $sess_user_id, 11, 2)) 
{
	...
	$startQuery = "SELECT";
	$valueQuery = "tgf.*, taf.insect_year, vs.aimag_name_mn, vs.aimag_name_en, vs.soum_name_mn, vs.soum_name_en 
	FROM ".$schemas.".tgforestinsect tgf, ".$schemas.".taforestinsect taf, scadministrative.vasoumname vs";
	
	if($sess_profile==1)
		$whereQuery = "WHERE tgf.insect_id = taf.insect_id AND taf.soum_code = vs.soum_code AND tgf.gid = ".$gid;
	else
		$whereQuery = "WHERE tgf.insect_id = taf.insect_id AND taf.soum_code = vs.soum_code AND tgf.gid = ".$gid." AND tgf.user_id = ".$sess_user_id;
...
{
    document.getElementById("updateforestinsectgeombttn").value = "1";
...
<div class="table-responsive">
  <table class="table table-bordered table-striped">
    <thead>
      <tr>
        <th><?php echo _p("EditText3")." "._p("GisSubTitle1")." "._p("EditText4"); ?></th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><div class="alert alert-warning" role="alert"><?php echo _p("AlertText"); ?></div>
          <form class="form" action="<?php echo $my_url.$my_page.$search_url.$sort_url; ?>" enctype="multipart/form-data" method="post" name="mainform" id="mainform">
            ...
            <div class="form-group row">
              <label class="col-md-3 col-form-label"><?php echo _p("GisSub1Column3"); ?>:</label>
              <div class="col-md-4">
                <input type="text" readonly class="form-control-plaintext" value="<?php echo $row[$i]["insect_year"]; ?>"/>
              </div>
            </div>				
            <div class="form-group row">
              <label class="col-md-3 col-form-label"><?php echo _p("GisSub1Column1"); ?>:</label>
              <div class="col-md-4">
                <input type="text" readonly class="form-control-plaintext" value="<?php echo $row[$i]["aimag_name_$language_name"]; ?>"/>
              </div>
            </div>
			 <div class="form-group row">
              <label class="col-md-3 col-form-label"><?php echo _p("GisSub1Column2"); ?>:</label>
              <div class="col-md-4">
			  ...
			  <div class="form-group row col-md-12">
			    <table>
				 <tbody>
				    <tr>
					  <th style="width: 5%">№</th>
					  <th style="width: 35%"><?php echo _p("GisSub1ColumnText1");?></th>
					  <th style="width: 10%"><?php echo _p("GisSub1ColumnText2");?></th>
					  <th><?php echo _p("GisSub1ColumnText3");?></th>
					  <th><?php echo _p("Description");?></th>
				    </tr>
					<tr>
					  <th>1.</th>
					  <td><?php echo _p("GisSub1Column4");?></td>
					  <td><?php echo _p("GisSub1Unit2");?></td>
					...
					<tr>
					  <th>2.</th>
					  <td><?php echo _p("GisSub1Column5");?></td>
					  <td><?php echo _p("GisSub1Unit1");?></td>
					...
					<tr>
					  <th>3.</th>
					  <td><?php echo _p("GisSub1Column6");?></td>
					  <td><?php echo _p("GisSub1Unit1");?></td>
					  <td>
					    <div class="form-check form-check-inline">
						  <input class="form-check-input" type="radio" name="control_type" id="control_type" value="1" <?php if($row[$i]["control_type"]==1) echo "checked"; ?>>
						  <label class="form-check-label" for="inlineCheckbox1"><?php echo _p("GisSub1MoreColumn1");?></label>
						</div>
						<div class="form-check form-check-inline">
						  <input class="form-check-input" type="radio" name="control_type" id="control_type" value="2" <?php if($row[$i]["control_type"]==2) echo "checked"; ?>>
						  <label class="form-check-label" for="inlineCheckbox2"><?php echo _p("GisSub1MoreColumn2");?></label>
						</div>
						<div class="form-check form-check-inline">
						  <input class="form-check-input" type="radio" name="control_type" id="control_type" value="3" <?php if($row[$i]["control_type"]==3) echo "checked"; ?>>
						  <label class="form-check-label" for="inlineCheckbox3"><?php echo _p("GisSub1MoreColumn3");?></label>
						</div></td>
					  <td><span class="form-text text-muted"><?php echo _p("AlertTextCheckbox"); ?></span></td>
					</tr>
					<tr>
					  <th>4.</th>
					  <td><?php echo _p("GisSub1Column7");?></td>
					  <td><?php echo _p("GisSub1Unit2");?></td>
					  <td><input type="text" class="form-control" name="control_area" id="control_area" value="<?php echo $row[$i]["control_area"]; ?>"/></td>
					  <td><span class="form-text text-muted"><?php echo _p("AlertTextReal11"); ?></span></td>
					</tr>
					<tr>
					  <th>5.</th>
            ...
            <input type="hidden" id="updateforestinsectgeombttn" name="updateforestinsectgeombttn" value="0"/>
