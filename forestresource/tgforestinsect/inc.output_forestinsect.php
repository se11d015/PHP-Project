...
$startQuery = "SELECT";
$valueQuery1 = "tgf.gid, taf.insect_year, vs.aimag_name_mn, vs.aimag_name_en, vs.soum_name_mn, vs.soum_name_en, tgf.insect_name, tgf.insect_damage_area";
if ($outputtype == 1)
...
$valueQuery3 = "FROM ".$schemas.".tgforestinsect tgf, ".$schemas.".taforestinsect taf, scadministrative.vasoumname vs";
...
if (!empty($row)) {
    if ($outputtype == 1 || $outputtype == 3) {
        $buffer = "";
        $buffer .= "№: ".$row[$i]["gid"]."\r\n";
        $buffer .= _p("GisSub1Column3").": ".$row[$i]["insect_year"]."\r\n";
        $buffer .= _p("GisSub1Column1").": ".$row[$i]["aimag_name_$language_name"]."\r\n";
        $buffer .= _p("GisSub1Column2").": ".$row[$i]["soum_name_$language_name"]."\r\n";
        $buffer .= _p("GisSub1Column4").": ".$row[$i]["insect_damage_area"]."\r\n";
        $buffer .= _p("GisSub1Column5").": ".$row[$i]["insect_name"]."\r\n";
        $buffer .= _p("GeometryText").": ".$row[$i]["geomtext"]."\r\n";

        $filename = "forestinsect.txt";
    ...
    if ($outputtype == 2) {
        ...
        $kml[] = '<value><![CDATA[' . $row[$i]["insect_year"] . ']]></value>';
        $kml[] = '</Data>';
        ...
        $kml[] = '<value><![CDATA[' . $row[$i]["insect_damage_area"] . ']]></value>';
        $kml[] = '</Data>';
        $kml[] = '<Data name="'._p("GisSub1Column5").'">';
        $kml[] = '<value><![CDATA[' . $row[$i]["insect_name"] . ']]></value>';
        $kml[] = '</Data>';
        $kml[] = '</ExtendedData>';
