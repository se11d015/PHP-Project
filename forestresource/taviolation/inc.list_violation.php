...
if (isset($_GET["sort"])) 
{
	...
	{
        $sortQuery .= " taf.forfeit_cost";
    } else if ($_GET["sort"] == 7) 
	{
        $sortQuery .= " taf.indemnity_cost";		
    } else
	{
...
$startQuery = "SELECT";
$valueQuery = "COUNT(*) AS num_count FROM ".$schemas.".taviolation taf, scadministrative.vasoumname vs";
...
require("taviolation/inc.search_violation.php");	
...
<div class="table-responsive">
  <table id="forestresource_datatables" class="table table-bordered table-hover" title_name="<?php echo _p("ResourceSubTitle8"); ?>" file_name="violation" column_name="0, 1, 2, 3, 4, 5, 6, 7" language_name="<?php echo $language_name;?>" page_count="<?php echo $count;?>">
    <thead>
      <tr>
      ...
      <tr>
        ...
        <th><a href="<?php echo $my_url."&sort=6&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="fa fa-sort"></i> <?php echo _p("ResourceSub8Column6"); ?></a></th>		
        <th><a href="<?php echo $my_url."&sort=7&sorttype=".$sorttype.$search_url; ?>" style="color:#FFFFFF"><i class="fa fa-sort"></i> <?php echo _p("ResourceSub8Column7"); ?></a></th>		
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?php echo $rows[$i]["forfeit_cost"]; ?></td>
        <td><?php echo $rows[$i]["indemnity_cost"]; ?></td>
        <td><a href="<?php echo $my_url.$my_page.$search_url.$sort_url."&action=more&violation_id=".$rows[$i]["violation_id"]; ?>" title="<?php echo _p("MoreTitle"); ?>"><i class="fa fa-list-alt"></i></a>
          <?php 
			if($sess_profile==1)
			{ 
			?>
          <a href="<?php echo $my_url.$my_page.$search_url.$sort_url."&action=edit&violation_id=".$rows[$i]["violation_id"]; ?>" title="<?php echo _p("EditTitle"); ?>"><i class="fa fa-pencil"></i></a> <a href="<?php echo $my_url.$search_url.$sort_url."&action=delete&violation_id=".$rows[$i]["violation_id"]; ?>" title="<?php echo _p("DeleteTitle"); ?>" onClick="return confirm('<?php echo _p("DeleteText3"); ?>');"><i class="fa fa-trash"></i></a>
          <?php
