<div class="content-wrapper">
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col">
        <h1> <?php echo _p("GisSubTitle2"); ?> </h1>
		  <?php
			if ($db->isGroupRole($schemas, $sess_profile, $sess_user_id, 12, 1)) 
			{
				...
				require("tgreforestation/inc.admin_export_reforestation.php");
				
				if ($sess_profile==1 || $db->isGroupRole($schemas, $sess_profile, $sess_user_id, 12, 2)) 
				{
					require("modules/generategeom.class.php");

					if (isset($_POST["insertreforestationgeombttn"]) && (int) $_POST["insertreforestationgeombttn"] == 1) 
					{
						if (isset($_POST["reforest_id"]) && isset($_POST["reforest_type"]))
						{
							$reforest_id = (int) $_POST["reforest_id"];
							$reforest_type = (int) $_POST["reforest_type"];
								
							$reforest_area = floatval(str_replace(",",".",$_POST["reforest_area"]));
							...
							$fields = array("reforest_id", "reforest_type", "reforest_area", "geom", "user_id");
							
							$checkvalues = array($reforest_id, $reforest_type, $reforest_area);
							...
							$result = $db->insert("".$schemas.".tgreforestation", $fields, $values);
							if(! $result)
					...
					if (isset($_POST["updatereforestationgeombttn"]) && (int) $_POST["updatereforestationgeombttn"] == 1) 
					{
			     
						if (isset($_POST["reforest_id"]) && isset($_POST["reforest_type"]) && isset($_POST["gid"]))
						{
							...
							if($geomtype==6)
							{
								$fields = array("reforest_id", "reforest_type", "reforest_area", "user_id");
							...
							$result = $db->update("".$schemas.".tgreforestation", $fields, $values, $wherevalues);
							if (!$result) {
					...
					if (($action == "delete") && isset($_GET["gid"])) 
					{
						...
						$result = $db->delete("".$schemas.".tgreforestation", $wherevalues);
						if (!$result)
				...
				{
					require("tgreforestation/inc.select_admin_reforestation.php");
				}elseif ($action=="export")
				{
					require("tgreforestation/inc.export_reforestation.php");
				} elseif ($action == "more")
				{
					require("tgreforestation/inc.more_reforestation.php");
				} elseif ($action == "outputgeom")
				{
					require("tgreforestation/inc.output_reforestation.php");					
				} else
				{
					require("tgreforestation/inc.list_reforestation.php");
				}
			} else {
