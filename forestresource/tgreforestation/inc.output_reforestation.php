...
$startQuery = "SELECT";
$valueQuery1 = "tgf.gid, taf.reforest_year, vs.aimag_name_mn, vs.aimag_name_en, vs.soum_name_mn, vs.soum_name_en, tgf.reforest_area";
if ($outputtype == 1)
...
$valueQuery3 = "FROM ".$schemas.".tgreforestation tgf, ".$schemas.".tareforestation taf, scadministrative.vasoumname vs";
...
if (!empty($row)) {
    if ($outputtype == 1 || $outputtype == 3) {
        $buffer = "";
        $buffer .= "№: ".$row[$i]["gid"]."\r\n";
        $buffer .= _p("GisSub2Column3").": ".$row[$i]["reforest_year"]."\r\n";
        $buffer .= _p("GisSub2Column1").": ".$row[$i]["aimag_name_$language_name"]."\r\n";
        $buffer .= _p("GisSub2Column2").": ".$row[$i]["soum_name_$language_name"]."\r\n";
        $buffer .= _p("GisSub2Column5").": ".$row[$i]["reforest_area"]."\r\n";;
        $buffer .= _p("GeometryText").": ".$row[$i]["geomtext"]."\r\n";

        $filename = "reforestation.txt";
    ...
    if ($outputtype == 2) {
        ...
        $kml[] = '<value><![CDATA[' . $row[$i]["reforest_year"] . ']]></value>';
        $kml[] = '</Data>';
        ...
        $kml[] = '<value><![CDATA[' . $row[$i]["reforest_area"] . ']]></value>';
        $kml[] = '</Data>';
        $kml[] = '</ExtendedData>';
