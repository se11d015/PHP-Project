	...
	if ($sess_profile==1 || $db->isGroupRole($schemas, $sess_profile, $sess_user_id, 12, 2)) 
	{
		if (isset($_POST["excelbttn"]) && (int) $_POST["excelbttn"] == 1) 
		{
			...
			if(!empty($rows)) 
			{
				...
				$sheet->setCellValue($cellname[$cellcol].$cellrow, _p("GisSubTitle2"));
				$cellrow++; 

				$cellcol = 0;				
				$sheet->setCellValue($cellname[$cellcol].$cellrow, "№"); $cellcol++; 
				$sheet->setCellValue($cellname[$cellcol].$cellrow, _p("GisSub2Column3")); $cellcol++;
				$sheet->setCellValue($cellname[$cellcol].$cellrow, _p("GisSub2Column1")); $cellcol++;
				$sheet->setCellValue($cellname[$cellcol].$cellrow, _p("GisSub2Column2")); $cellcol++;
				$sheet->setCellValue($cellname[$cellcol].$cellrow, _p("GisSub2Column4")); $cellcol++;
				$sheet->setCellValue($cellname[$cellcol].$cellrow, _p("GisSub2Column5")); $cellcol++;
				$sheet->setCellValue($cellname[$cellcol].$cellrow, _p("GeometryText")); $cellcol++;
				
				$cellrow++;
				$cellcol = 0; 
				$cellcol = $cellcol+5;
				$sheet->setCellValue($cellname[$cellcol].$cellrow, _p("GisSub2Unit2")); $cellcol++;
				...
				{	
					...
					if($rows[$i]["reforest_type"]==1) $reforest_type = _p("GisSub2MoreColumn1"); 
					else if($rows[$i]["reforest_type"]==2) $reforest_type = _p("GisSub2MoreColumn2");
					else if($rows[$i]["reforest_type"]==3) $reforest_type = _p("GisSub2MoreColumn3");
					$sheet->setCellValue($cellname[$cellcol].$cellrow, $reforest_type); $cellcol++;
					$sheet->setCellValue($cellname[$cellcol].$cellrow, $rows[$i]["reforest_area"]); $cellcol++;
		...
		if (isset($_POST["kmlbttn"]) && (int) $_POST["kmlbttn"] == 1) 
		{
			...
			if(!empty($rows)) 
			{
				...
				{		
					...
					$kml[] = '<ExtendedData>';
					$kml[] = '<Data name="'._p("GisSub2Column3").'">';
					$kml[] = '<value><![CDATA[' . $rows[$i]["reforest_year"] . ']]></value>';
					$kml[] = '</Data>';
					$kml[] = '<Data name="'._p("GisSub2Column1").'">';
					$kml[] = '<value><![CDATA[' . $rows[$i]["aimag_name_$language_name"] . ']]></value>';
					$kml[] = '</Data>';
					$kml[] = '<Data name="'._p("GisSub2Column2").'">';
					$kml[] = '<value><![CDATA[' . $rows[$i]["soum_name_$language_name"] . ']]></value>';
					$kml[] = '</Data>';
					$kml[] = '<Data name="'._p("GisSub2Column5").'">';
					$kml[] = '<value><![CDATA[' . $rows[$i]["reforest_area"] . ']]></value>';
					$kml[] = '</Data>';											
					$kml[] = '</ExtendedData>';
					$kml[] = $rows[$i]['geomtext'];
					$kml[] = '</Placemark>';			
				...
				header('Content-Disposition: attachment; filename="reforestation.kml"');
				ob_clean();
				flush();
				echo $kmlOutput;
				exit;
